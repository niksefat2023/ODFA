...
byte* expand_bytes1(byte* source,int source_size,int expanded_size)
{
...
byte* expand_bytes(byte* source,int source_size,int expanded_size)
{
	...
	if (expanded_size>source_size)
	{
		...
		for (int i=source_size;i<expanded_size;i++)
		{
			expanded[i] = '\x0';
		}
	}
	...
	)
{
	...
	int message_bytes = 10-logq_bytes;
	...
	state_size = message_bytes + logq_bytes;
	int* state_sizes =  new int[input_size];
//	printf("message_bytes: %d - real_message_bytes: %d - state_size: %d",message_bytes,real_message_bytes,state_size);
	int length;
	int xx;
...
//	printf("message_bytes: %d - real_message_bytes: %d - state_size: %d",message_bytes,real_message_bytes,state_size);
	...
	//checking ckeys
//	for (int i=0;i<input_size;i++)
//		PrintBytes("key:",c_keys[i],state_size);
	...
	for (int i=0;i<input_size;i++)
	{
		...
		else
		{
			if (i<(input_size-1))
			{
				byte* expanded_pad = PRG_n_to_m_bytes_hash(pad_bytes,message_bytes,256*message_bytes);
				byte* expanded_alpha = new byte[message_bytes];
				temp = XOREncrypt(target,c_keys[i],state_size);
				for (int l=0;l<message_bytes;l++)
				{
					expanded_alpha[l] = expanded_pad[inputs[i]*message_bytes+l];
				}
				cur_state=XOREncrypt(temp,PRG_n_to_m_bytes_hash(expanded_alpha,message_bytes,state_size),state_size);
				divide_bytes(cur_state,pad_bytes,message_bytes,NS_bytes,logq_bytes);
				NS = convert_byte_to_int(logq_bytes,NS_bytes);
				cout << "NS is:" << NS << endl;
				if (NS>dfa_size)
...
//	PrintBytes ("result is: ",result,message_bytes);
	// Check if it is the expanded message so we can shrink it
	byte* shrinked_result;
	if (message_bytes != real_message_bytes)
	{

		shrinked_result = expand_bytes(result,message_bytes,real_message_bytes);
		delete [] result;
	}
	...
	//cout << "real_message_bytes is: " << real_message_bytes << endl;
	if (message_bytes>real_message_bytes)
	{
	...
	)
{
	...
	int real_message_bytes = (int)ceil((double)message_size/8);
	int message_bytes = 10-logq_bytes;
	byte** expanded_messages = new byte*[dfa_size];
	if (message_bytes<=real_message_bytes)
	{
		message_bytes = real_message_bytes;
	}
...
//	PrintBytes("key",keys[114][0],state_size);
//	PrintBytes("key",keys[97][0],state_size);
//	PrintBytes("key",keys[97][1],state_size);
	...
	for (int i=0;i<input_size;i++)
	{
		...
		for (int j=0;j<dfa_size;j++)
		{
			...
			else
			{
				if (i<(input_size-1))
				{
					byte* expanded_pad = PRG_n_to_m_bytes_hash(pad[i][j],message_bytes,256*message_bytes);
					byte* expanded_alpha = new byte[message_bytes];
					for (int z=0;z<256;z++)
					{
						PM[z][i][j] = convert_int_to_kbyte(logq_bytes,per[i+1][dfa[z][rper[i][j]]]);
						...
						for (int l=0;l<message_bytes;l++)
						{
							expanded_alpha[l] = expanded_pad[z*message_bytes+l];
						}

						prg_output = PRG_n_to_m_bytes_hash(expanded_alpha,message_bytes,state_size);
				...
				else
				{
					...
					for (int z=0;z<256;z++)
					{
						PM[z][i][j] = convert_int_to_kbyte(logq_bytes,1);
						if (message_bytes == real_message_bytes)
							comb_output = combine_bytes(messages[dfa[z][rper[i][j]]],message_bytes,PM[z][i][j],logq_bytes);
						else
